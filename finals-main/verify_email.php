...
if (isset($_GET['token'])) {
    ...
    $stmt = $conn->prepare("SELECT user_id FROM stud_users WHERE verification_token = ? AND is_verified = 0");
    ...
    if ($stmt->num_rows > 0) {
        ...
        $stmt = $conn->prepare("UPDATE stud_users SET is_verified = 1, verification_token = NULL WHERE user_id = ?");
